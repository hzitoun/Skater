...
class TestDNNI(unittest.TestCase):
    ...
    def test_deep_interpreter_cnn(self):
        ...
        with DeepInterpreter(session=K.get_session()) as di:
            ...
            relevance_scores_occ = di.explain('occlusion', target_tensor * ys, input_tensor, xs, use_case='image')
            # test for optional arguments related to window_size and step size for 'occlusion'
            optional_args = {"window_size": 2,
                             "step": 2}
            relevance_scores_occ_2 = di.explain('occlusion', target_tensor * ys, input_tensor, xs,
        ...
        self.assertEquals(relevance_scores_occ_2.shape, (2, 28, 28, 1))
